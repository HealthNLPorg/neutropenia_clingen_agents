...
// targeted sections
Pathogenic||PATHOGENIC\h+SINGLE\h+NUCLEOTIDE\h+VARIANTS\h+AND\h+SMALL\h+INSERTIONS/DELETIONS[:\*]$
Potential Germline Pathogenic||POTENTIAL\s+GERMLINE\s+PATHOGENIC\s+SINGLE\s+NUCLEOTIDE\s+VARIANTS\s+AND\s+SMALL\s+INSERTIONS/DELETIONS:$
Potential Germline VUS||POTENTIAL\h+GERMLINE\h+VARIANTS\h+OF\h+UNKNOWN\h+SIGNIFICATION\h+(VUS):$
Other VUS||OTHER\h+VARIANTS\h+OF\h+UNKNOWN\h+SIGNIFICANCE\h+(VUS)*:$
// non-targeted sections typically after targeted sections for filtering
Potential Germline with Possible Secondary Somatic Variants||POTENTIAL\s+GERMLINE\s+\\(WITH\s+POSSIBLE\s+SECONDARY\s+SOMATIC\\)\s+VARIANTS\s+\\(SBDS,\s+TERC,\s+TERT,\s+DKC[L1],\s+AND\s+DDX41\s+ONLY\\)*:$
Copy Number Analysis||COPY\h+NUMBER\h+ANALYSIS*:$
